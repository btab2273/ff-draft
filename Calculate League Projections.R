...
# -Still having problems with density plots
# -ESPN projections do not include fumbles!
#To do:
#-Add FantasyPros Projections
#FantasyPros Projections added
# Set up for Livin' the Dream
# Removed objects for receptions
###########################
...
projections <- merge(projections_espn, projections_cbs, all=TRUE)
...
projections <- merge(projections, projections_nfl, all=TRUE)
...
projections <- merge(projections, projections_fp, all=TRUE)
...
#projections$rec_espn <- projections$rec_espn*recMultiplier       #new variable for receptions
...
#projections$rec_cbs <- projections$rec_cbs*recMultiplier
...
#projections$rec_nfl <- projections$rec_nfl
...
#projections$recPts_nfl <- NA
#projections_nfl$recPts_nfl <- as.numeric(projections_nfl$rec_nfl)
...
#projections$rec_fp <- projections$rec_fp
...
projections$passYds <- rowMeans(projections[,c("passYds_espn","passYds_cbs","passYds_fp")], na.rm=TRUE)
projections$passTds <- rowMeans(projections[,c("passTds_espn","passTds_cbs","passTds_fp")], na.rm=TRUE)
projections$passInt <- rowMeans(projections[,c("passInt_espn","passInt_cbs","passInt_fp")], na.rm=TRUE)
projections$rushYds <- rowMeans(projections[,c("rushYds_espn","rushYds_cbs","rushYds_fp")], na.rm=TRUE)
projections$rushTds <- rowMeans(projections[,c("rushTds_espn","rushTds_cbs","rushTds_nfl","rushTds_fp")], na.rm=TRUE)
#projections$recs <- rowMeans(projections[,c("rec_espn","rec_cbs", "rec_fp")], na.rm=TRUE)
...
#for (i in 1:dim(projections)[1]){
#  ifelse(projections$passYds_espn[i]==0 | projections$passYds_cbs[i]==0, projections$passYds[i] <- max(projections$passYd#s_espn[i], projections$passYds_cbs[i], na.rm=TRUE), projections$passYds[i] <- projections$passYds[i])
#  ifelse(projections$passTds_espn[i]==0 | projections$passTds_cbs[i]==0, projections$passTds[i] <- max(projections$passTd#s_espn[i], projections$passTds_cbs[i], na.rm=TRUE), projections$passTds[i] <- projections$passTds[i])
#  ifelse(projections$passInt_espn[i]==0 | projections$passInt_cbs[i]==0, projections$passInt[i] <- max(projections$passIn#t_espn[i], projections$passInt_cbs[i], na.rm=TRUE), projections$passInt[i] <- projections$passInt[i])
#  ifelse(projections$rushYds_espn[i]==0 | projections$rushYds_cbs[i]==0, projections$rushYds[i] <- max(projections$rushYd#s_espn[i], projections$rushYds_cbs[i], na.rm=TRUE), projections$rushYds[i] <- projections$rushYds[i])
#  ifelse(projections$rushTds_espn[i]==0 | projections$rushTds_cbs[i]==0, projections$rushTds[i] <- max(projections$rushTd#s_espn[i], projections$rushTds_cbs[i], na.rm=TRUE), projections$rushTds[i] <- projections$rushTds[i])
#  ifelse(projections$recYds_espn[i]==0 | projections$recYds_cbs[i]==0, projections$recYds[i] <- max(projections$recYds_es#pn[i], projections$recYds_cbs[i], na.rm=TRUE), projections$recYds[i] <- projections$recYds[i])
#  ifelse(projections$recTds_espn[i]==0 | projections$recTds_cbs[i]==0, projections$recTds[i] <- max(projections$recTds_es#pn[i], projections$recTds_cbs[i], na.rm=TRUE), projections$recTds[i] <- projections$recTds[i])
#  ifelse(projections$fumbles_espn[i]==0 | projections$fumbles_cbs[i]==0, projections$fumbles[i] <- max(projections$fumble#s_espn[i], projections$fumbles_cbs[i], na.rm=TRUE), projections$fumbles[i] <- projections$fumbles[i])
#}
...
#projections[,c("name","rec_espn","rec_cbs","rec_fp", "rec")]           #removed to see if it stops crash (08/26/2013)
...
#subset(projections, select=-c(rec_NFL, recPts_nfl))



#projections <- projections[!is.na(projections$pts_fp),]
...
#projections$recPts <- projections$rec
...
#Remove Free Agents
#projections <- projections[!is.na(projections$team_fp=="NA" & projections$team_espn=="NA" & projections$team_cbs=="NA" & projections$team_nfl=="NA"),]
...
ggplot(densityData, aes(x=pointDensity, fill=sourceDensity, pseudocount=0.0001)) + geom_density(alpha=.3) + xlab("Player's Projected Points") + ggtitle("Density Plot of Projected Points for 2013") + theme(legend.title=element_blank())
